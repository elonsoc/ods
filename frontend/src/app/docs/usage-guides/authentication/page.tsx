...
const UGAuthentication = () => {
	return (
		<div className={styles.docsPageMainContent}>
			...
			<p>
				Authentication is a crucial step in interacting with the Open Data
				Service (ODS) API. This guide will walk you through the authentication
				process, ensuring secure access to the data about Elon University. Open
				Data Service utilizes a single sign-on (SSO) approach to be able to
				register for applications combined with an API key for authentication.
			</p>
			...
			<p>
				...
				credentials, you can authenticate and gain authorized access to the ODS
				API resources. This SSO approach ensures a seamless and secure
				authentication process.
			</p>
			<h2 id='Including_API_Key_in_Requests'>Including API Key in Requests</h2>
			<p>
				In addition to the SSO authentication, you also need to include your API
				key in the requests to authenticate your API calls. After logging in via
				SSO and obtaining your API key, you should include it in the
			...
			<p>
				The API key should be included using the "Bearer" authentication method,
				such as:
			</p>
			<p className='code'>Authorization: Bearer YOUR_API_KEY</p>
			<p>
				Ensure that you replace `YOUR_API_KEY` with the actual API key obtained
				from your registered application.
			</p>
			<h2 id='Authentication_Example'>Authentication Example</h2>
			<p>
				Here's an example of an API request using cURL, including the API key in
				the header:
			</p>
			<CodeCopyable
				code={`curl -X GET http://api.ods.elon.edu/v1/buildings/ -H "Authorization: Bearer YOUR_API_KEY"`}
			/>
			...
			<p>
				...
				authentication when interacting with the ODS API. By authenticating your
				...
				process. Enjoy seamless and secure access to the wealth of information
				provided by the Open Data Service API!
			</p>
